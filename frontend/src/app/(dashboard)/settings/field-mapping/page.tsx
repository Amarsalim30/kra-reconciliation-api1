...
export default function FieldMappingPage() {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || "Unable to retrieve settings.");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    return (
      <SettingsLayout>
        <div className="bg-white border border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center min-h-[300px] gap-3">
          ...
          <span className="text-sm font-medium text-slate-600">Loading SAP Field Mapping...</span>
        </div>
      </SettingsLayout>
    );
  }
      ...
      <SettingsLayout>
        <div className="p-6 bg-rose-50 border border-rose-200 rounded-xl text-rose-800 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-semibold text-base">Error Loading SAP Field Mapping</h3>
          </div>
  ...
  return (
    <SettingsLayout>
      <SAPFieldMappingCard
        settingsVersion={composite.system_settings.version}
