...
def get_purchases(
    ...
    sap_field_mappings = db.query(SAPFieldMapping).filter(SAPFieldMapping.module == VatModule.PURCHASES).all()
...
def upload_purchases_csv(
    ...
    expected_required_sections = {
        ...
        if (isinstance(config, str) and sec_id == "SEC_F") or
           (isinstance(config, dict) and config.get("active", True) and config.get("required", True) and config.get("module") == "purchases")
    }
    ...
    for file in files:
        ...
        matched_sec, sec_config = resolve_filename_to_section(filename, mappings, VatModule.PURCHASES)
        
        if not matched_sec:
            warnings.append(f"Skipped file '{filename}': Could not detect a valid KRA section identifier for Purchases.")
            continue
