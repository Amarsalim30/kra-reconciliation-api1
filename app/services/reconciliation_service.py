...
def compare_amount(sap: SalesInvoice, kra: SalesInvoice) -> Difference:
...
def compare_vat(sap: SalesInvoice, kra: SalesInvoice) -> Difference:
...
def compare_date(sap: SalesInvoice, kra: SalesInvoice) -> Difference:
...
def reconcile_sales(sap: list[SalesInvoice], kra: list[SalesInvoice]) -> tuple[ReconciliationSummary, list[ReconciliationResult]]:
    """
    Executes the O(n) reconciliation matching algorithm on SAP and KRA invoices.
    Returns the compiled summary metrics and line-by-line comparison results.
    """
    sap_by_cu: dict[str, list[SalesInvoice]] = {}
    kra_by_cu: dict[str, list[SalesInvoice]] = {}
    ...
    for cu in all_cus:
        ...
        if len(sap_list) > 1 or len(kra_list) > 1:
            ...
            # Select first as representative for debugging
